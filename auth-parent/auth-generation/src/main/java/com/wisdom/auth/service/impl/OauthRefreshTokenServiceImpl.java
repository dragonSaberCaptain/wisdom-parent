package com.wisdom.auth.service.impl;

import com.wisdom.auth.entity.OauthRefreshToken;
import com.wisdom.auth.dao.OauthRefreshTokenDao;
import com.wisdom.auth.service.OauthRefreshTokenService;
import com.wisdom.common.service.impl.BaseServiceImpl;
import org.springframework.stereotype.Service;
import org.springframework.beans.factory.annotation.Autowired;

/**
 *  逻辑层
 *
 * @author captain
 * @version 1.0.0
 */
@Service
public class OauthRefreshTokenServiceImpl extends BaseServiceImpl<OauthRefreshTokenDao, OauthRefreshToken> implements OauthRefreshTokenService {

}
